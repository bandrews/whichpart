...
| GPIO count | 39 in the 48-pin package, all mappable to external interrupt vectors; most are 5 V tolerant, but every pin wired to an ADC channel is marked \`TTa\` — 3.3 V tolerant only [1] | Slightly more usable I/O than the F103 in the same package. The 5 V tolerance is not universal, and the exceptions are exactly the analogue-capable pins you might want to put a 5 V sensor on. |
| Notable peripherals | 2× SPI (18 Mbit/s), 2× I²C with Fast Mode Plus at 1 Mbit/s and 20 mA sink, 2× USART, one 12-bit 1.0 µs ADC with 10 external and 2 internal channels, seven timers on this device (one advanced-control, five general-purpose, one basic) plus SysTick and two watchdogs, calendar RTC, 5-channel DMA, CRC unit, SWD [1] | Fast Mode Plus I²C with 20 mA sink is a real convenience for driving longer or more heavily loaded I²C buses. ST's "11 timers" headline covers the whole F030 family; this device has seven timer peripherals. |
...
**Only one ADC, with 10 external channels in this package** (plus two internal
ones for the temperature sensor and reference). The F103 has two independent
ADCs; if your design relies on simultaneous dual-channel sampling, this part
cannot do it. [1]

**Five-volt tolerance has a specific exception.** ST's pin table classifies every
I/O: \`FT\` is 5 V tolerant, \`FTf\` is 5 V tolerant and Fast-Mode-Plus capable, and
\`TTa\` is "3.3 V tolerant I/O directly connected to ADC". So the pins you would
use for analogue inputs are the ones that will not survive a 5 V signal. If you
need both a 5 V-tolerant pin and an analogue reading, they cannot be the same
pin. [1]
...
1. STMicroelectronics, *STM32F030x4 STM32F030x6 STM32F030x8 STM32F030xC —
   ...
   Features page 1, Section 2 (Description), Table 2 (family device features and
   peripheral counts), Table 10 (Legend/abbreviations used in the pinout table),
   Table 11 (pin definitions). <https://www.st.com/resource/en/datasheet/stm32f030c8.pdf>
