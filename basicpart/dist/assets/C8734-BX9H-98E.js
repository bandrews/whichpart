...
| GPIO count | 37 in the 48-pin package, almost all 5 V tolerant, all mappable to 16 external interrupt vectors [1] | The 5 V tolerance is the reason this chip survives so much hobbyist abuse — but "almost all" means you must check the pin table. |
...
| Operating temperature | −40 °C to +85 °C ambient for the \`6\` order-code suffix, which is what the \`T6\` in \`C8T6\` denotes; junction −40 °C to +125 °C [1] | The \`7\` suffix version goes to +105 °C, but that is a different part number. |
