...
| Operating temperature | −40 °C to +85 °C, the range over which the characteristics are specified [1] | Industrial range. |
...
**The oscillator capacitor is integrated, and that fixes which crystal to
buy.** You supply the 32.768 kHz crystal but not its load capacitors, which is
two fewer parts and two fewer things to get wrong. In exchange, the crystal has
to match what the chip provides: NXP specifies a parallel-resonant crystal with a
10 pF load capacitance and a series resistance no higher than 40 kΩ. Fit a 12.5 pF
crystal — much the more common type on a catalogue page — and the clock will run
fast. The integrated load capacitance itself is 25 pF typical with a 15–35 pF
spread, which is why NXP documents three different ways of adjusting the
frequency. The simplest — picking one fixed capacitor for your layout and leaving
it — is the one it says will "easily" reach ±5 minutes a year. [1]
