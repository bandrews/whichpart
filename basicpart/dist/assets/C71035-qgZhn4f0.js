...
| Input offset voltage | 2 mV typical, 7 mV maximum at 25 °C for the plain LM324 grade, 9 mV maximum over its temperature range; drift 7 µV/°C typical, 30 µV/°C maximum [1] | Loose. Fine for a comparator-ish job or an AC signal; not for precision DC. Note the catalog's 5 mV is the tighter LM124/LM224 limit, not this grade's. [2] |
...
| Output swing | Common-mode input range 0 V to V<sub>CC</sub> − 1.5 V at 25 °C, narrowing to V<sub>CC</sub> − 2 V over temperature; output source current 40 mA typical, sink current 20 mA typical at 15 V [1] | The input goes to ground but stops 1.5 V short of the positive rail — a very common source of confusion — and stops further short when hot or cold. |
| Supply current | For all four amplifiers, no load: 0.7 mA typical at 25 °C and 0.8 mA typical over temperature, 1.2 mA maximum, at V<sub>CC</sub> = 5 V; 1.5 mA typical and 3 mA maximum at 30 V [1] | Around 200 µA per amplifier — genuinely low power for four channels. The draw roughly doubles at 30 V. |
...
over temperature.** With a 100 kΩ source resistance that worst case is 20 mV of
error — nearly three times the part's own 7 mV offset limit. Match the impedance
seen by each input if DC accuracy matters. ST also notes the current flows *out*
of the input pins, which decides which way the error goes. [1]

**Output crossover distortion is the LM324's known weakness, and TI documents
why.** The measured figure looks fine — total harmonic distortion is 0.015 %
typical at 1 kHz with 20 dB of gain, measured at 2 V peak-to-peak into 2 kΩ. But
TI's application note for this family explains that the output stage gives its
transistors no static bias at all (a class-B arrangement, where most op-amps use
class AB), so the output takes a moment to hand over between its sourcing and
sinking devices as the signal crosses zero. TI's own fix is the traditional one:
a pull-down or pull-up resistor at the output that keeps one driver permanently
conducting. For audio, this is why people reach for the NE5532 or TL072
instead. [1] [3]
...
200 nA over-temperature maximum for the LM324 grade. The 5 mV offset figure is
also optimistic for this order code — ST guarantees 5 mV on the LM124 and LM224
grades but 7 mV on the LM324, so design to 7 mV. [1] [2]
...
3. Texas Instruments, *Application Design Guidelines for LM324 and LM358
   Devices*, SLOA277B, January 2019, revised July 2023. Section 4.3 (Output
   Crossover Time Delay). <https://www.ti.com/lit/pdf/sloa277>
`;export{e as default};