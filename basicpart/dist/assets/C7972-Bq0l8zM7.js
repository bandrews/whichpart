...
| Supply current | LMV321I grade: 80 µA typical and 170 µA maximum on a 2.7 V rail; 130 µA typical, 250 µA maximum at 25 °C and 350 µA maximum over temperature on a 5 V rail [1] | Low enough for battery-powered designs that need an always-on amplifier. TI publishes a separate table per supply, and the 5 V numbers are roughly 60 % higher — budget from the table that matches your rail. |
...
**Rail-to-rail applies to the output, not the input.** With CMRR held at
50 dB or better, TI guarantees a common-mode input range of 0 V to 1.7 V on a
2.7 V supply and 0 V to 4 V on a 5 V supply — in both cases roughly a volt short
of the top rail, though the inputs do reach ground. Design so the inputs sit in
the lower part of the supply range. [1]
...
to +125 °C. Three of these come from the 2.7 V table specifically — the 11 nA
bias current and 46 nV/√Hz noise are its typicals, and the 170 µA
quiescent-current entry is its *maximum* (the typical is 80 µA). On a 5 V rail
the part draws more: 130 µA typical, 250 µA maximum. The 1 V/µs slew rate and
63 dB CMRR are typicals. [1] [2]
...
1. Texas Instruments, *LMV358, LMV321, LMV324, LMV324S — General-Purpose,
   ...
   (Electrical Characteristics: V<sub>CC+</sub> = 2.7 V) and Section 6.8
