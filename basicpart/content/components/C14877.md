...
- **Product status is unclear.** Some distributors flag `ATMEGA328P-AU` as NRND
  ("not recommended for new designs"), while other authorized channels list it as
  active with a projected end-of-life well into the future. We could not confirm
  a status directly from a Microchip primary source, so treat the repository's
  existing "not recommended for new designs" note as unverified rather than
  established. See `ISSUES.md`.
